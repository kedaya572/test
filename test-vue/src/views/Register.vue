<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <h2>创建账号</h2>
      <p class="subtitle">注册后即可管理学生信息</p>
      ...
      <form @submit.prevent="handleRegister">
        ...
        <div class="field">
          ...
          <div class="input-wrap">
            ...
            <input v-model="form.password" type="password" placeholder="请输入密码" required autocomplete="new-password" />
          </div>
        </div>
...
const handleRegister = async () => {
  ...
  try {
    ...
    errMsg.value = err.message || '注册失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
...
.auth-logo span { font-size: 16px; font-weight: 600; color: var(--text); }
h2 { font-size: 22px; font-weight: 700; color: var(--text); margin-bottom: 4px; }
...
.input-wrap input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(79,70,229,.1); }
...
.switch-link { text-align: center; font-size: 13px; color: var(--text-muted); margin-top: 20px; }
</style>
